...
class KLKHHaulRoadController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $baseQuery = DB::table('prd_klkh_haulroad as hr')
        ...
        //         $query->where('hr.foreman', $user->nik)
        //             ->orWhere('hr.supervisor', $user->nik)
        //             ->orWhere('hr.superintendent', $user->nik);
        //     });
        // }
    ...
    {
        $hr = DB::table('prd_klkh_haulroad as hr')
        ->leftJoin('users as us', 'hr.pic', '=', 'us.id')
        ->leftJoin('ref_region as ar', 'hr.pit_id', '=', 'ar.id')
        ->leftJoin('ref_shift as sh', 'hr.shift_id', '=', 'sh.id')
        ->leftJoin('users as us3', 'hr.foreman', '=', 'us3.nik')
