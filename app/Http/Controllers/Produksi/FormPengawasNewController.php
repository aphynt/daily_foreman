...
class FormPengawasNewController extends Controller
{
    ...
    public function users(Request $request)
    {
        ...
        $data['users'] = User::when(!empty($roleForeman), function ($q) use ($roleForeman) {
                $q->whereIn('role_id', $roleForeman);
            })
            ->get();
    ...
    public function post(Request $request)
    {
        ...
        Log::create([
            ...
            'nik' => Auth::user()->nik,
            'keterangan' => 'Tambah laporan kerja dengan nama: '. Auth::user()->name . ', NIK: '. Auth::user()->nik .
    ...
    public function preview($uuid)
    {
        ...
        $front = DB::table('prd_daily_foreman_front_loading as fl')
        ->leftJoin('prd_daily_foreman_daily_report as dr', 'fl.daily_report_id', '=', 'dr.id')
        ->leftJoin('focus.focus.dbo.FLT_VEHICLE as flt', 'fl.nomor_unit', '=', 'flt.VHC_ID')
        ->select(
            'fl.nomor_unit',
        ...
        $catatan = DB::table('prd_daily_foreman_note as cp')
        ->leftJoin('prd_daily_foreman_daily_report as dr', 'cp.daily_report_id', '=', 'dr.id')
        ->select(
            'cp.jam_start',
            'cp.jam_stop',
            'cp.keterangan',
