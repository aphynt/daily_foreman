...
class FrontLoadingController extends Controller
{
    ...
    ->flatMap(function ($item) {
        ...
        $result = [];
        $siangKeteranganIndex = 0;
        $malamKeteranganIndex = 0;

        if ($item->shift !== 'Siang') {
            foreach ($malam as $waktu) {
                $keteranganMalam = $ket[$malamKeteranganIndex] ?? '';
                $result[] = [
                    'id' => $item->id,
        ...
        if ($item->shift !== 'Malam') {
            foreach ($siang as $waktu) {
                $keteranganSiang = $ket[$siangKeteranganIndex] ?? '';
                $result[] = [
                    'id' => $item->id,
                    'tanggal_pelaporan' => $item->tanggal_pelaporan,
                    'shift' => $item->shift,
