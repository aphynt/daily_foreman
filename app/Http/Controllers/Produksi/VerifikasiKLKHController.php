...
class VerifikasiKLKHController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $loading = DB::table('prd_klkh_loadingpoint as lp')
        ->leftJoin('users as us', 'lp.pic', '=', 'us.id')
        ->leftJoin('ref_region as ar', 'lp.pit_id', '=', 'ar.id')
        ->leftJoin('ref_shift as sh', 'lp.shift_id', '=', 'sh.id')
        ->leftJoin('focus.focus.dbo.PRS_PERSONAL as gl', 'lp.foreman', '=', 'gl.NRP')
        ->leftJoin('focus.focus.dbo.PRS_PERSONAL as spv', 'lp.supervisor', '=', 'spv.NRP')
        ->leftJoin('focus.focus.dbo.PRS_PERSONAL as spt', 'lp.superintendent', '=', 'spt.NRP')
        ->select(
            'lp.id',
            'lp.uuid',
            'lp.pic as pic_id',
        ...
        $user = Auth::user();
        $roleGroup = getConfigArrayById(9) ?? [];

        if (in_array($user->role_id, $roleGroup)) {

            switch ($user->role_id) {

                case 6: // FOREMAN
                    $loading->where('foreman', $user->nik);
                    break;

                case 5: // SUPERVISOR
                    $loading->where('supervisor', $user->nik);
                    break;

                case 4: // SUPERINTENDENT
                    $loading->where('superintendent', $user->nik);
                    break;
            }

        } elseif ($user->role_id == 1) {

            $loading->orWhere('pic', $user->id);
        }

        $haulroad = DB::table('prd_klkh_haulroad as hr')
        ->leftJoin('users as us', 'hr.pic', '=', 'us.id')
        ->leftJoin('ref_region as ar', 'hr.pit_id', '=', 'ar.id')
        ->leftJoin('ref_shift as sh', 'hr.shift_id', '=', 'sh.id')
        ->leftJoin('focus.focus.dbo.PRS_PERSONAL as gl', 'hr.foreman', '=', 'gl.NRP')
        ->leftJoin('focus.focus.dbo.PRS_PERSONAL as spv', 'hr.supervisor', '=', 'spv.NRP')
        ->leftJoin('focus.focus.dbo.PRS_PERSONAL as spt', 'hr.superintendent', '=', 'spt.NRP')
        ->select(
            'hr.id',
            'hr.uuid',
            'hr.pic as pic_id',
        ...
        if (in_array($user->role_id, $roleGroup)) {
            ...
            switch ($user->role_id) {
                ...
                case 6: // FOREMAN
                    $haulroad->where('foreman', $user->nik);
                    break;

                case 5: // SUPERVISOR
                    $haulroad->where('supervisor', $user->nik);
                    break;

                case 4: // SUPERINTENDENT
                    $haulroad->where('superintendent', $user->nik);
                    break;
            }

        } elseif ($user->role_id == 1) {

            $haulroad->orWhere('pic', $user->id);
        }

        $disposal = DB::table('prd_klkh_disposal as dp')
        ->leftJoin('users as us', 'dp.pic', '=', 'us.id')
        ->leftJoin('ref_region as ar', 'dp.pit_id', '=', 'ar.id')
        ->leftJoin('ref_shift as sh', 'dp.shift_id', '=', 'sh.id')
        ->leftJoin('focus.focus.dbo.PRS_PERSONAL as gl', 'dp.foreman', '=', 'gl.NRP')
        ->leftJoin('focus.focus.dbo.PRS_PERSONAL as spv', 'dp.supervisor', '=', 'spv.NRP')
        ->leftJoin('focus.focus.dbo.PRS_PERSONAL as spt', 'dp.superintendent', '=', 'spt.NRP')
        ->select(
            'dp.id',
            'dp.uuid',
            'dp.pic as pic_id',
        ...
        if (in_array($user->role_id, $roleGroup)) {
            ...
            switch ($user->role_id) {
                ...
                case 6: // FOREMAN
                    $disposal->where('foreman', $user->nik);
                    break;

                case 5: // SUPERVISOR
                    $disposal->where('supervisor', $user->nik);
                    break;

                case 4: // SUPERINTENDENT
                    $disposal->where('superintendent', $user->nik);
                    break;
            }

        } elseif ($user->role_id == 1) {

            $disposal->orWhere('pic', $user->id);
        }

        $lumpur = DB::table('prd_klkh_lumpur as lum')
        ->leftJoin('users as us', 'lum.pic', '=', 'us.id')
        ->leftJoin('ref_region as ar', 'lum.pit_id', '=', 'ar.id')
        ->leftJoin('ref_shift as sh', 'lum.shift_id', '=', 'sh.id')
        ->leftJoin('focus.focus.dbo.PRS_PERSONAL as gl', 'lum.foreman', '=', 'gl.NRP')
        ->leftJoin('focus.focus.dbo.PRS_PERSONAL as spv', 'lum.supervisor', '=', 'spv.NRP')
        ->leftJoin('focus.focus.dbo.PRS_PERSONAL as spt', 'lum.superintendent', '=', 'spt.NRP')
        ->select(
            'lum.id',
            'lum.uuid',
            'lum.pic as pic_id',
        ...
        if (in_array($user->role_id, $roleGroup)) {
            ...
            switch ($user->role_id) {
                ...
                case 6: // FOREMAN
                    $lumpur->where('foreman', $user->nik);
                    break;

                case 5: // SUPERVISOR
                    $lumpur->where('supervisor', $user->nik);
                    break;

                case 4: // SUPERINTENDENT
                    $lumpur->where('superintendent', $user->nik);
                    break;
            }

        } elseif ($user->role_id == 1) {

            $lumpur->orWhere('pic', $user->id);
        }

        $ogs = DB::table('prd_klkh_ogs as ogs')
        ->leftJoin('users as us', 'ogs.pic', '=', 'us.id')
        ->leftJoin('ref_region as ar', 'ogs.pit_id', '=', 'ar.id')
        ->leftJoin('ref_shift as sh', 'ogs.shift_id', '=', 'sh.id')
        ->leftJoin('focus.focus.dbo.PRS_PERSONAL as gl', 'ogs.foreman', '=', 'gl.NRP')
        ->leftJoin('focus.focus.dbo.PRS_PERSONAL as spv', 'ogs.supervisor', '=', 'spv.NRP')
        ->leftJoin('focus.focus.dbo.PRS_PERSONAL as spt', 'ogs.superintendent', '=', 'spt.NRP')
        ->select(
            'ogs.id',
            'ogs.uuid',
            'ogs.pic as pic_id',
        ...
        if (in_array($user->role_id, $roleGroup)) {
            ...
            switch ($user->role_id) {
                ...
                case 6: // FOREMAN
                    $ogs->where('foreman', $user->nik);
                    break;

                case 5: // SUPERVISOR
                    $ogs->where('supervisor', $user->nik);
                    break;

                case 4: // SUPERINTENDENT
                    $ogs->where('superintendent', $user->nik);
                    break;
            }

        } elseif ($user->role_id == 1) {

            $ogs->orWhere('pic', $user->id);
        }

        $batubara = DB::table('prd_klkh_batubara as lp')
        ->leftJoin('users as us', 'lp.pic', '=', 'us.id')
        ->leftJoin('ref_region as ar', 'lp.pit_id', '=', 'ar.id')
        ->leftJoin('ref_shift as sh', 'lp.shift_id', '=', 'sh.id')
        ->leftJoin('focus.focus.dbo.PRS_PERSONAL as gl', 'lp.foreman', '=', 'gl.NRP')
        ...
        if (in_array($user->role_id, $roleGroup)) {
            ...
            switch ($user->role_id) {
                ...
                case 6: // FOREMAN
                    $batubara->where('foreman', $user->nik);
                    break;

                case 5: // SUPERVISOR
                    $batubara->where('supervisor', $user->nik);
                    break;

                case 4: // SUPERINTENDENT
                    $batubara->where('superintendent', $user->nik);
                    break;
            }

        } elseif ($user->role_id == 1) {

            $batubara->orWhere('pic', $user->id);
        }

        $simpangempat = DB::table('prd_klkh_simpangempat as se')
        ->leftJoin('users as us', 'se.pic', '=', 'us.id')
        ->leftJoin('ref_region as ar', 'se.pit_id', '=', 'ar.id')
        ->leftJoin('ref_shift as sh', 'se.shift_id', '=', 'sh.id')
        ->leftJoin('focus.focus.dbo.PRS_PERSONAL as gl', 'se.foreman', '=', 'gl.NRP')
        ->leftJoin('focus.focus.dbo.PRS_PERSONAL as spv', 'se.supervisor', '=', 'spv.NRP')
        ->leftJoin('focus.focus.dbo.PRS_PERSONAL as spt', 'se.superintendent', '=', 'spt.NRP')
        ->select(
            'se.id',
            'se.uuid',
            'se.pic as pic_id',
        ...
        if (in_array($user->role_id, $roleGroup)) {
            ...
            switch ($user->role_id) {
                ...
                case 6: // FOREMAN
                    $simpangempat->where('foreman', $user->nik);
                    break;

                case 5: // SUPERVISOR
                    $simpangempat->where('supervisor', $user->nik);
                    break;

                case 4: // SUPERINTENDENT
                    $simpangempat->where('superintendent', $user->nik);
                    break;
            }

        } elseif ($user->role_id == 1) {

            $simpangempat->orWhere('pic', $user->id);
        }
