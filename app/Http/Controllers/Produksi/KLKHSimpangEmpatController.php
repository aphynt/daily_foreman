...
class KLKHSimpangEmpatController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $baseQuery = DB::table('prd_klkh_simpangempat as se')
        ...
        if (!$user->hasRoleId($roleBypassAdminManagement)) {
            ...
            $baseQuery->where(function($query) use ($user) {
                $query->where('se.foreman', $user->nik)
                    ->orWhere('se.supervisor', $user->nik)
                    ->orWhere('se.superintendent', $user->nik);
            });
        }
    ...
    {
        $se = DB::table('prd_klkh_simpangempat as se')
        ->leftJoin('users as us', 'se.pic', '=', 'us.id')
        ->leftJoin('ref_region as ar', 'se.pit_id', '=', 'ar.id')
        ->leftJoin('ref_shift as sh', 'se.shift_id', '=', 'sh.id')
        ->leftJoin('users as us3', 'se.foreman', '=', 'us3.nik')
