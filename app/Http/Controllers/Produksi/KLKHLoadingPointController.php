...
class KLKHLoadingPointController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $baseQuery = DB::table('prd_klkh_loadingpoint as lp')
        ...
        //         $query->where('lp.foreman', $user->nik)
        //             ->orWhere('lp.supervisor', $user->nik)
        //             ->orWhere('lp.superintendent', $user->nik);
        //     });
        // }
    ...
    {
        $ld = DB::table('prd_klkh_loadingpoint as lp')
        ->leftJoin('users as us', 'lp.pic', '=', 'us.id')
        ->leftJoin('ref_region as ar', 'lp.pit_id', '=', 'ar.id')
        ->leftJoin('ref_shift as sh', 'lp.shift_id', '=', 'sh.id')
        ->leftJoin('users as us3', 'lp.foreman', '=', 'us3.nik')
    ...
    {
        $lp = DB::table('prd_klkh_loadingpoint as lp')
        ->leftJoin('users as us', 'lp.pic', '=', 'us.id')
        ->leftJoin('ref_region as ar', 'lp.pit_id', '=', 'ar.id')
        ->leftJoin('ref_shift as sh', 'lp.shift_id', '=', 'sh.id')
        ->leftJoin('users as us3', 'lp.foreman', '=', 'us3.nik')
