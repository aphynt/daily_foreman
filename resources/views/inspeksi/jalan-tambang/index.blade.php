@include('layout.head', ['title' => 'Inspeksi Jalan Tambang'])
                ...
                <div class="row align-items-center">
                    <div class="col-md-12">
                        <ul class="breadcrumb">
                            ...
                            <li class="breadcrumb-item"><a href="javascript: void(0)">Jalan Tambang</a></li>
                        </ul>
                    </div>
                    <div class="col-12">
                        <div class="mb-3 d-flex flex-wrap flex-md-nowrap align-items-center justify-content-between gap-2">
                            ...
                            <div class="d-flex flex-wrap gap-2">
                                ...
                                {{-- FOREMAN & SUPERVISOR --}}
                                @if (canAccess('inspeksi.jalantambang.insert'))
                                    <a href="{{ route('inspeksi.jalantambang.insert') }}">
                                        <span class="badge bg-success px-3 py-2" style="font-size:14px">
                                            <i class="fas fa-plus"></i> Isi Inspeksi Jalan Tambang
                                        </span>
                                    </a>
                                @endif

                                {{-- ADMIN & MANAGEMENT --}}
                                @if (canAccess('inspeksi.jalantambang.bundlepdf'))
                                    <a href="{{ route('inspeksi.jalantambang.bundlepdf') }}" target="_blank">
                            ...
                            <table id="cbtn-selectors" class="table table-striped table-hover table-bordered nowrap">
                                <thead style="text-align: center; vertical-align: middle;">
                                    <tr>
                                        ...
                                        <th colspan="2">Penanggung Jawab Area</th>
                                ...
                                <tbody>
                                    @foreach ($jt as $item)
                                        <tr>
                                            ...
                                            <td>{{ $item->nik_pic }}</td>
                                            <td>{{ $item->pic }}</td>
                                            <td>{{ date('d-m-Y', strtotime($item->tanggal_inspeksi)) }}
                                            <td>{{ $item->pit }}</td>
                                            <td>{{ $item->nama_lokasi }}</td>
                                            @if ($item->nama_penanggungjawab != null)
                                                @if ($item->verified_penanggungjawab == null)
                                                    <td>{{ $item->nik_penanggungjawab }}</td>
                                                    <td>{{ $item->nama_penanggungjawab }} <span class="badge bg-danger">B</span></td>
                                                @else
                                                    <td>{{ $item->nik_penanggungjawab }}</td>
                                                    <td>{{ $item->nama_penanggungjawab }} <span class="badge bg-success">T</span></td>
                                                @endif
                                            @else
                                                <td>{{ $item->nik_penanggungjawab }}</td>
                                            ...
                                            <td>
                                                <a href="{{ route('inspeksi.jalantambang.preview', $item->uuid) }}" class="avtar avtar-s btn btn-primary btn-sm">
                                                ...
                                                <a href="#" class="avtar avtar-s btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#deleteJT{{$item->id}}">
                                        ...
                                        @include('inspeksi.jalan-tambang.delete')
                                    @endforeach
