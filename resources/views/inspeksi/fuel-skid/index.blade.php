@include('layout.head', ['title' => 'Inspeksi Fuel Skid'])
                ...
                <div class="row align-items-center">
                    <div class="col-md-12">
                        <ul class="breadcrumb">
                            ...
                            <li class="breadcrumb-item"><a href="javascript: void(0)">Fuel Skid</a></li>
                        </ul>
                    </div>
                    <div class="col-12">
                        <div class="mb-3 d-flex flex-wrap flex-md-nowrap align-items-center justify-content-between gap-2">
                            ...
                            <div class="d-flex flex-wrap gap-2">
                                ...
                                {{-- FOREMAN & SUPERVISOR --}}
                                @if (canAccess('inspeksi.fuelskid.insert'))
                                    <a href="{{ route('inspeksi.fuelskid.insert') }}">
                            ...
                            <table id="cbtn-selectors" class="table table-striped table-hover table-bordered nowrap">
                                <thead style="text-align: center; vertical-align: middle;">
                                    <tr>
                                        ...
                                        <th rowspan="2">Nomor Tangki</th>
                                        <th rowspan="2">Kapasitas Tangki</th>
                                ...
                                <tbody>
                                    @foreach ($fs as $item)
                                        <tr>
                                            ...
                                            <td>{{ $item->pic }}</td>
                                            <td>{{ $item->lokasi }}</td>
                                            <td>{{ $item->nomor_tangki }}</td>
                                            <td>{{ $item->kapasitas_tangki }}</td>
                                            <td>{{ date('d-m-Y', strtotime($item->tanggal_inspeksi)) }} {{ date('H:i', strtotime($item->jam_inspeksi)) }}</td>

                                            @if ($item->nik_dibuat != null)
                                                @if ($item->verified_dibuat == null)
                                                    <td>{{ $item->nik_dibuat }}</td>
                                                    <td>{{ $item->nama_dibuat }} <span class="badge bg-danger">B</span></td>
                                                @else
                                                    <td>{{ $item->nik_dibuat }}</td>
                                                    <td>{{ $item->nama_dibuat }} <span class="badge bg-success">T</span></td>
                                                @endif
                                            @else
                                                <td>{{ $item->nik_dibuat }}</td>
                                                <td>{{ $item->nama_dibuat }}</td>
                                            @endif

                                            @if ($item->nik_diperiksa != null)
                                                @if ($item->verified_diperiksa == null)
                                                    <td>{{ $item->nik_diperiksa }}</td>
                                                    <td>{{ $item->nama_diperiksa }} <span class="badge bg-danger">B</span></td>
                                                @else
                                                    <td>{{ $item->nik_diperiksa }}</td>
                                                    <td>{{ $item->nama_diperiksa }} <span class="badge bg-success">T</span></td>
                                                @endif
                                            @else
                                                <td>{{ $item->nik_diperiksa }}</td>
                                                <td>{{ $item->nama_diperiksa }}</td>
                                            @endif
                                            <td>
                                                <a href="{{ route('inspeksi.fuelskid.preview', $item->uuid) }}" class="avtar avtar-s btn btn-primary btn-sm">
                                                ...
                                                @if ( $item->pic == Auth::user()->id || in_array(Auth::user()->role, [ 'ADMIN' ]))
                                                <a href="#" class="avtar avtar-s btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#deleteFS{{$item->id}}">
                                        ...
                                        @include('inspeksi.fuel-skid.delete')
                                    @endforeach
