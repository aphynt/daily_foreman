@include('layout.head', ['title' => 'KLKH Troubleshooting IT'])
                ...
                <div class="row align-items-center">
                    <div class="col-md-12">
                        <ul class="breadcrumb">
                            ...
                            <li class="breadcrumb-item"><a href="javascript: void(0)">Troubleshooting IT</a></li>
                        </ul>
                    </div>
                    <div class="col-12">
                        <div class="mb-3 d-flex flex-wrap flex-md-nowrap align-items-center justify-content-between gap-2">
                            ...
                            <div class="d-flex flex-wrap gap-2">
                                ...
                                {{-- FOREMAN & SUPERVISOR --}}
                                @if (canAccess('klkh.it.insert'))
                                    <a href="{{ route('klkh.it.insert') }}">
                                        <span class="badge bg-success px-3 py-2" style="font-size:14px">
                                            <i class="fas fa-plus"></i> Isi KLKH
                                        </span>
                                    </a>
                                @endif

                                {{-- ADMIN & MANAGEMENT --}}
                                {{-- @if (canAccess('klkh.it.bundlepdf'))
                                    <a href="{{ route('klkh.it.bundlepdf') }}" target="_blank">
                                        ...
                                        <tr>
                                            ...
                                            @if ($item->verified_diketahui == null)
                                                <td>{{ $item->nik_diketahui }}</td>
                                                 <td>{{ $item->nama_diketahui }} <span class="badge bg-danger">B</span></td>
                                            @else
                                                <td>{{ $item->nik_diketahui }}</td>
                                                <td>{{ $item->nama_diketahui }} <span class="badge bg-success">T</span></td>
                                            @endif

                                            <td>
                                                <a href="{{ route('klkh.it.preview', $item->uuid) }}" class="avtar avtar-s btn btn-primary btn-sm">
                                                ...
                                                <a href="#" class="avtar avtar-s btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#deleteIT{{$item->id}}">
